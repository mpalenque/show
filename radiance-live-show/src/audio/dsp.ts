...
export function createDspState(frameSize = 4096): DspState {
  if (frameSize < 2 || (frameSize & (frameSize - 1)) !== 0) {
    throw new Error('FFT frame size must be a power of two.');
  }
  return {
    frameSize,
...
export function extractFeatures(
  ...
  if (samples.length !== state.frameSize) throw new Error(`Expected ${state.frameSize} samples.`);
  ...
  for (let bin = 1; bin < bins; bin += 1) {
    ...
    if (frequency >= 27.5 && frequency <= 5000) {
      ...
      state.chroma[((midi % 12) + 12) % 12] += power;
    }
  }
  ...
  // Relative spectral shares are useful for colour, but by themselves they
  // stay high in silence. Gate them with calibrated RMS before publishing.
  const gatedBand = (power: number): number => clamp01(Math.sqrt(power / bandTotal) * 1.4 * normalizedRms);

  return {
    t,
    rawRms,
    rms: normalizedRms,
    bands: { low: gatedBand(low), mid: gatedBand(mid), high: gatedBand(high) },
    onset,
    onsetStrength,
    chroma: new Float32Array(state.chroma),
    centroid: normalizedRms > 0.001
      ? clamp01((weightedFrequency / (total + 1e-12)) / nyquist * 2)
      : 0,
    flux: flux * normalizedRms,
    noteAttacks,
  };
}

function detectNoteAttacks(
  state: DspState,
  sampleRate: number,
  onset: boolean,
  t: number,
  ...
  for (let index = 0; index < PIANO_NOTE_COUNT; index += 1) {
    ...
    const score = fundamental * 0.84
      + harmonicEnergy / Math.max(harmonicWeight, 1e-8) * 0.16;
