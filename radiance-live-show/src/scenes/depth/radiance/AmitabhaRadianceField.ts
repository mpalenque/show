...
const MAX_BODIES = 48;
// Match the high-quality Box2D/HRC reference. Work is distributed across
// display frames by RadianceOverlay, so the sorter still presents at video
// cadence while the 1024² transport field resolves asynchronously.
...
const sceneFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Raster coverage maps to the same near-opaque density used by the
    // reference Box2D bodies (roughly 6-9). Dark bars therefore block the
    // field instead of behaving like dim, transparent emitters.
    vec3 absorption = vec3(clamp(externalScene.a * 8.0, 0.0, 8.0));
...
const frustumSeedFragmentShader = /* glsl */ `
  ...
  uniform float uPureExternalEmission;
  ...
  void main() {
    ...
    vec4 emissivitySample = texture(uEmissivity, samplePosition);
    vec3 emissivity = emissivitySample.rgb;
    ...
    // Radiance 2 uses mathematically pure emitters: sorted pixels inject
    // energy without becoming absorbing bodies. The legacy/Box2D paths keep
    // the original density-coupled seed when this route-only switch is zero.
    radiance += emissivity
      * uPureExternalEmission
      * step(0.00001, emissivitySample.a);
...
const mergeConesFragmentShader = /* glsl */ `
  ...
  float farAttenuation() {
    float normalizedReach = clamp(uReach / 1.5, 0.0, 1.0);
    if (normalizedReach >= 0.999) return 1.0;
    float reachPixels = mix(24.0, 512.0, pow(normalizedReach, 1.35));
    float segmentDistance = exp2(uCascadeIndex);
    return exp2(-segmentDistance / max(reachPixels, 1.0));
  }
  ...
  void mergeRadiance(
    ...
    radiance = nearRadiance + farRadiance * nearTransmit * farAttenuation();
    transmit = nearTransmit * farTransmit;
  }
  ...
  void mergeCone(
    ...
    if (mod(plane, 2.0) == 0.0) {
      ...
      radiance = rayRadiance * coneWeight
        + farConeRadiance * rayTransmit * farAttenuation();
...
const displayFragmentShader = /* glsl */ `
  ...
  uniform float uTransparentLightScale;
  uniform float uTransparentShadowStrength;
  ...
  vec3 sampleIrradiance(vec2 uv) {
    return mix(
      texture(uPreviousDisplayIrradiance, uv).rgb,
      texture(uIrradiance, uv).rgb,
      uTemporalBlend
    );
  }
  ...
  void main() {
    ...
    // HRC transports through a square metric. Embed the rectangular browser
    // viewport in that square before sampling so a light cone stays isotropic
    // instead of being stretched into a horizontal or vertical ray.
    vec2 viewportUv = uDisplayViewport.xy + vUv * uDisplayViewport.zw;
    vec2 screenPosition = mix(uWorldBounds.xy, uWorldBounds.zw, viewportUv);
    ...
    if (!outsideField) {
      ...
      vec2 resolveTexel = uFieldTexel * max(uResolveRadius, 1.0);
      vec2 dx = vec2(resolveTexel.x, 0.0);
      vec2 dy = vec2(0.0, resolveTexel.y);
      vec3 center = sampleIrradiance(sampleUv);
      vec3 west = sampleIrradiance(sampleUv - dx);
      vec3 east = sampleIrradiance(sampleUv + dx);
      vec3 north = sampleIrradiance(sampleUv + dy);
      vec3 south = sampleIrradiance(sampleUv - dy);
      vec3 northWest = sampleIrradiance(sampleUv - dx + dy);
      vec3 northEast = sampleIrradiance(sampleUv + dx + dy);
      vec3 southWest = sampleIrradiance(sampleUv - dx - dy);
      vec3 southEast = sampleIrradiance(sampleUv + dx - dy);
      ...
      // Compare against a wider light neighbourhood. A local radiance deficit
      // with nearby energy is a transported shadow; uniformly unlit regions
      // stay transparent instead of becoming a global black veil.
      vec2 shadowTexel = uFieldTexel * max(10.0, uResolveRadius * 7.0);
      vec2 shadowDx = vec2(shadowTexel.x, 0.0);
      vec2 shadowDy = vec2(0.0, shadowTexel.y);
      float nearbyLight = max(
        max(
          colourLuma(sampleIrradiance(clamp(sampleUv - shadowDx, uFieldTexel, vec2(1.0) - uFieldTexel))),
          colourLuma(sampleIrradiance(clamp(sampleUv + shadowDx, uFieldTexel, vec2(1.0) - uFieldTexel)))
        ),
        max(
          colourLuma(sampleIrradiance(clamp(sampleUv - shadowDy, uFieldTexel, vec2(1.0) - uFieldTexel))),
          colourLuma(sampleIrradiance(clamp(sampleUv + shadowDy, uFieldTexel, vec2(1.0) - uFieldTexel)))
        )
      );
      float shadowDeficit = clamp(
        (nearbyLight - colourLuma(radiance)) / max(nearbyLight, 0.02),
        0.0,
        1.0
      );
      shadowMask = smoothstep(0.12, 0.72, shadowDeficit)
        * smoothstep(0.015, 0.12, nearbyLight);

    }
    vec3 background = vec3(0.0);
    if (uBackgroundEnabled > 0.5) {
      // Keep the cloud image atmospheric so the Radiance Cascade still owns
      // the contrast, emitters and shadows instead of being washed out.
      background = mix(
        vec3(0.002, 0.006, 0.016),
        texture(uBackgroundTexture, vUv).rgb * vec3(0.34, 0.54, 0.84),
        uBackgroundOpacity
      );
    }
    vec3 exposed = background + radiance * 1.45;
    vec3 mapped = toneMapPreservingHue(exposed);
    vec3 displayColour = pow(mapped, vec3(1.0 / 2.2));
    if (uTransparentOutput > 0.5) {
      vec3 lightColour = displayColour * clamp(uTransparentLightScale, 0.0, 1.0);
      float lightAlpha = clamp(max(max(lightColour.r, lightColour.g), lightColour.b), 0.0, 1.0);
      float shadowAlpha = clamp(shadowMask * uTransparentShadowStrength, 0.0, 0.85);
      // Premultiplied coloured light and black shadow share one physical layer.
      // Only a local transported deficit can raise alpha without RGB.
      outColour = vec4(lightColour, max(lightAlpha, shadowAlpha));
      return;
    }
    outColour = vec4(displayColour, 1.0);
  }
`;
...
export class AmitabhaRadianceField {
  ...
  constructor(private readonly renderer: THREE.WebGLRenderer, initialQuality: 'high' | 'safe' = 'high') {
    const initialResolution = initialQuality === 'high' ? HIGH_FIELD_EXTENT : SAFE_FIELD_EXTENT;
    this.fieldExtent = initialResolution;
    this.cascadeCount = Math.ceil(Math.log2(initialResolution));
    this.allocateTargets(initialResolution);
    ...
    this.displayMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uPreviousDisplayIrradiance: { value: this.fieldTargets[0].texture },
        uTemporalBlend: { value: 1 },
        ...
        uDisplayViewport: { value: new THREE.Vector4(0, 0, 1, 1) },
        ...
        uResolveRadius: { value: 1 },
        uTransparentOutput: { value: 0 },
        uTransparentLightScale: { value: 0.48 },
        uTransparentShadowStrength: { value: 0.5 },
      },
  ...
  setTemporalBlend(amount: number): void {
    this.displayMaterial.uniforms.uTemporalBlend.value = Math.max(0, Math.min(1, amount));
  }
  ...
  setDisplayViewport(x: number, y: number, width: number, height: number): void {
    this.displayMaterial.uniforms.uDisplayViewport.value.set(
      Math.max(0, Math.min(1, x)),
      Math.max(0, Math.min(1, y)),
      Math.max(0.0001, Math.min(1, width)),
      Math.max(0.0001, Math.min(1, height)),
    );
  }
  ...
  render(frustaPerStep = this.frustumsPerStep): void {
    ...
    if (this.nextFrustum >= FRUSTUM_COUNT) {
      ...
      this.displayMaterial.uniforms.uPreviousDisplayIrradiance.value = previousTexture;
      this.displayMaterial.uniforms.uIrradiance.value = this.texture;
      this.displayMaterial.uniforms.uTemporalBlend.value = 0;
  ...
  private renderFrustum(frustumIndex: number): void {
    ...
    for (let level = this.cascadeCount - 1; level >= 0; level -= 1) {
      ...
      if (previousMerge) {
        ...
        // Samplers are active even while uHasFar is false. Bind the unused far
        // input to the opposite ping-pong target so the first merge never reads
        // from its own framebuffer attachment.
        const safeFar = this.mergeTargets[1 - (level % 2)];
        this.mergeMaterial.uniforms.uFarRadiance.value = safeFar.textures[0];
        this.mergeMaterial.uniforms.uFarTransmit.value = safeFar.textures[1];
      }
  ...
  setPureExternalEmission(active: boolean): void {
    this.seedMaterial.uniforms.uPureExternalEmission.value = active ? 1 : 0;
  }

  setExternalBounceGain(amount: number): void {
    this.sceneMaterial.uniforms.uBounceGain.value = Math.max(
      0,
      Math.min(0.5, amount),
    );
  }

  setTransportReach(amount: number): void {
    this.mergeMaterial.uniforms.uReach.value = Math.max(
      0.05,
      Math.min(1.5, amount),
    );
  }

  setDisplayBlurRadius(radius: number): void {
    this.displayMaterial.uniforms.uResolveRadius.value = Math.max(
      1,
      Math.min(12, radius),
    );
  }

  setTransparentOutput(active: boolean): void {
    this.displayMaterial.uniforms.uTransparentOutput.value = active ? 1 : 0;
  }

  setTransparentComposite(lightScale: number, shadowStrength: number): void {
    this.displayMaterial.uniforms.uTransparentLightScale.value = Math.max(
      0,
      Math.min(1, lightScale),
    );
    this.displayMaterial.uniforms.uTransparentShadowStrength.value = Math.max(
      0,
      Math.min(1, shadowStrength),
    );
  }
  ...
  private clearTargets(): void {
    ...
    this.fieldReadIndex = 0;
    this.displayMaterial.uniforms.uIrradiance.value = this.texture;
    this.displayMaterial.uniforms.uPreviousDisplayIrradiance.value = this.texture;
    this.displayMaterial.uniforms.uTemporalBlend.value = 1;
  ...
  private updateTargetUniforms(): void {
    ...
    this.displayMaterial.uniforms.uPreviousDisplayIrradiance.value = this.fieldTargets[0].texture;
    this.displayMaterial.uniforms.uTemporalBlend.value = 1;
