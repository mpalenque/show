...
**Fluids, decisión vigente:** **24 = previa negra con sólo una línea blanca**, timeline detenido en cero y física vacía/preparada; **25 = PLAY del timeline completo desde cero**; **26 = el motor libre, que se toca por MIDI con `fluids.live.*`**. Notas **24/25/26 del canal 10**, sin reiniciar por notas repetidas; entrar desactiva loops de ensayo. **El audio del show lo reproduce la página** (con el track por Ableton los dos relojes se separaban): silenciar esa pista en Ableton y **hacer un clic en la ventana de salida** antes del show, que es lo que Chrome pide para dejar sonar. Al terminar la secuencia el fluido **sigue corriendo**. **27–29 libres**. [Operación actual](docs/integracion-radiance/OPERACION.md).
