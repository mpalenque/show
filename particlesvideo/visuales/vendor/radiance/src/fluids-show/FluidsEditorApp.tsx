...
import AudioTransport, { type AudioPeaks } from './audio-transport';
import CurveLane, { nextShape } from './lanes/CurveLane';
import EventLane from './lanes/EventLane';
import GestureLane from './lanes/GestureLane';
import WaveformLane from './lanes/WaveformLane';
import {
  clearStoredDoc,
  docHasEdits,
  downloadDoc,
...
import { parseAnalysis, seedShowDoc, type FluidsAnalysis } from './seed';
import {
  CURVE_SPECS,
  EVENT_SPECS,
  GESTURE_MODES,
...
import { useShowDoc } from './use-show-doc';
import './fluids-editor.css';

/**
 * Página del show "Fluids": editor de timeline, preview y modo perform en una
 * sola vista local.
 *
 * El stage repite el patrón de la página de Tres Masas — frame lógico fijo,
 * escalera SSAA anclada a píxeles de dispositivo, montaje único a prueba de
 * StrictMode — porque es el mismo motor y la misma resolución de show. Lo que
 * agrega esta página es el reloj: el tiempo que ve la escena sale del audio,
 * no de contar frames, y por eso la sincronía no deriva en dos minutos y
 * medio de track.
 */

const FS_WIDTH = 2688;
const FS_HEIGHT = 1008;
const FS_QUALITY: QualityLevel = 'high';
...
type FluidSceneWithShow = VisualScene & {
...
export default function FluidsEditorApp() {
  const stageRef = useRef<HTMLDivElement>(null);
  const sceneRef = useRef<FluidSceneWithShow | null>(null);
  const timelineRef = useRef<HTMLDivElement>(null);
  const playheadRef = useRef<HTMLDivElement>(null);
  const transportRef = useRef<AudioTransport | null>(null);
  if (transportRef.current === null) {
    transportRef.current = new AudioTransport(FLUIDS_AUDIO_PATH, FLUIDS_DURATION);
  }
  ...
  const [saveState, setSaveState] = useState<'limpio' | 'guardando' | 'guardado'>('limpio');
  const fileInputRef = useRef<HTMLInputElement>(null);
  /**
   * El autosave se saltea el cambio que viene de cargar. Sin esto, la primera
   * visita escribiría en el navegador el JSON commiteado y a partir de ahí
   * ganaría siempre la copia local, aunque el show del repo hubiera cambiado.
   */
  const skipNextSave = useRef(true);
  /**
   * De dónde salió el documento. Si no había nada guardado ni commiteado, la
   * página se siembra sola apenas llega el análisis: abrir /fluids y ver un
   * lienzo en blanco no dice nada, y obliga a saber que existe un botón.
   */
  ...
  const loopRef = useRef<{ range: typeof loop; on: boolean }>({ range: loop, on: loopOn });
  loopRef.current = { range: loop, on: loopOn };
  ...
  useEffect(() => {
    transport.setVolume(blackout ? 0 : 1);
  }, [blackout, transport]);
  ...
  // Carga inicial: lo que quedó en el navegador, si no el JSON commiteado, y
  // si no un documento vacío. Adoptar no deja historia: deshacer apenas se
  // abre la página no tiene que devolver a un show vacío.
  useEffect(() => {
    let cancelled = false;
    ...
    if (stored) {
      ...
      adopt(stored);
      setLoadState('guardado');
      return () => { cancelled = true; };
    }
    void (async () => {
      const shipped = await fetchShippedDoc();
      if (cancelled) return;
      if (!shipped) {
        setLoadState('vacio');
        return;
  ...
  // Autosave con freno de 500 ms: arrastrar una curva no puede serializar el
  // documento entero sesenta veces por segundo.
  useEffect(() => {
  ...
  // El documento llega a la escena por referencia; al reemplazarlo (editar,
  // deshacer, sembrar) hay que volver a entregarlo.
  useEffect(() => {
    sceneRef.current?.setFluidsShowDoc?.(doc);
  }, [doc, version]);

  // Primera visita sin nada guardado: sembrar es el default útil.
  useEffect(() => {
    if (bootstrapped.current || loadState !== 'vacio' || !analysis) return;
    bootstrapped.current = true;
    skipNextSave.current = true;
    adopt(seedShowDoc(analysis));
  }, [adopt, analysis, loadState]);
  ...
  const onFrame = useCallback((clock: number, dt: number) => {
    ...
    if (scene?.setFluidsLiveGesture) {
      if (pointer.active) {
        scene.setFluidsLiveGesture({
          mode: gestureMode,
          x: pointer.x,
          y: pointer.y,
        ...
        scene.setFluidsLiveGesture(null);
      }
    }
  ...
  useEffect(() => {
    ...
    let destroyed = false;
    let raf = 0;
    let scene: FluidSceneWithShow | null = null;
    ...
    let stageDeviceWidth = FS_WIDTH;

    const renderScaleFor = (): number => {
      const oneToOne = clamp(stageDeviceWidth / FS_WIDTH, 0.5, MAX_RENDER_SCALE);
    ...
    const measureStage = (): void => {
      ...
      if (dpr !== appliedDpr) {
        appliedDpr = dpr;
        host.style.setProperty('--fs-max-width', `${FS_WIDTH / dpr}px`);
      }
      const rect = host.getBoundingClientRect();
      const next = Math.max(1, Math.round(rect.width * dpr));
      if (next === stageDeviceWidth) return;
      stageDeviceWidth = next;
      scene?.resize(FS_WIDTH, FS_HEIGHT, renderScaleFor());
    };

    // Una media query de resolución es la señal confiable del cambio de ratio:
    // resize y el ResizeObserver disparan con devicePixelRatio todavía viejo.
    ...
    const boot = async () => {
      try {
        const created = await createScene('fluid', host, FS_QUALITY) as FluidSceneWithShow;
        if (destroyed) {
          created.dispose();
          return;
        }
        ...
        created.resize(FS_WIDTH, FS_HEIGHT, renderScaleFor());
        created.setFluidsShowDoc?.(store.doc);
        created.enter?.('fluids-show');
    ...
    const tick = (now: number) => {
      ...
      // Loop: rebobinar al llegar al final de la región marcada.
      const { range, on } = loopRef.current;
      if (on && range && transport.playing && transport.time >= range.to) {
        transport.seek(range.from);
      }
      ...
      try {
        scene.frame({
          ...
          look: 'fluids-show',
          params: {
            fluidsShowTime: clock,
            fluidsShowPlaying: transport.playing ? 1 : 0,
            fluidsShowGeoGain: 1,
          },
          audio: EMPTY_AUDIO,
          quality: FS_QUALITY,
        ...
        if (consecutiveErrors >= 5) {
          ...
          try { scene.dispose(); } catch { /* El error de render sigue siendo el principal. */ }
          scene = null;
      ...
      // El playhead se mueve con un transform, no redibujando las lanes: es
      // la única cosa que cambia a 60 fps y no tiene que costar un repintado.
      ...
      // El reloj visible se refresca a ~16 Hz: re-renderizar React a 60 fps
      // sólo para mover dos dígitos le come cuadros al show.
      if (now - lastReadoutAt >= READOUT_MS) {
        lastReadoutAt = now;
        setTime(clock);
        setPlaying(transport.playing);
      }
      ...
      if (now - lastStatsAt >= MEASURE_WINDOW_MS) {
        ...
        setStats({
          fps,
          particles: Number(canvas?.dataset.fluidsShowParticles ?? 0),
          pps: Number(canvas?.dataset.fluidsShowPps ?? 0),
          capped: Boolean(canvas?.dataset.fluidsShowCapped),
          warning: canvas?.dataset.fluidsShowWarning ?? '',
    ...
    return () => {
      ...
      try { scene?.dispose(); } catch { /* El desmontaje no puede dejar la página colgada. */ }
      scene = null;
      sceneRef.current = null;
    };
    // El loop lee valores vivos por refs; tiene que montarse una sola vez.
  ...
  }, [deleteSelection, mutate, redo, seek, selection, togglePlay, transport, undo]);
  ...
  return (
    <main className={`fs-shell ${perform ? 'fs-perform' : ''}`}>
      ...
      <div className="fs-fade" style={{ opacity: blackout ? 1 : 1 - master }} />
      ...
      {perform && (
        <section className="fs-panel fs-panel-min">
          ...
          <button className={`fs-btn fs-btn-danger ${blackout ? 'fs-btn-on' : ''}`}
            aria-pressed={blackout} onClick={() => setBlackout((value) => !value)}>
      ...
      <section className="fs-panel fs-editor-only">
        <div className="fs-transport">
          ...
          <input
            ...
            onChange={(event) => {
              void importDoc(event.target.files?.[0]);
          ...
          <button className="fs-btn" onClick={() => sceneRef.current?.resetFluidsShow?.()}>
            RESET FLUIDO
          </button>
        ...
        <div className="fs-stats">
          ...
          {loadState === 'guardado' && (
            <span className="fs-warn">
              DOC GUARDADO EN ESTE NAVEGADOR — SEMBRAR LO REEMPLAZA
            </span>
          )}
          <span className="fs-hint">
      ...
      <div className="fs-timeline fs-editor-only" ref={timelineRef}>
