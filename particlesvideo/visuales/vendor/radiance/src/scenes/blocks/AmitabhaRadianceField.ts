...
const MAX_BODIES = 48;
// The unified renderer exposes explicit 256², 512² and 1024² transport tiers.
// The HRC algorithm and pass formats remain identical at every quality.
  ...
  private renderFrustum(frustumIndex: number): void {
    ...
    for (let level = this.cascadeCount - 1; level >= 0; level -= 1) {
      ...
      if (previousMerge) {
        ...
        // WebGL rejects a draw whenever any bound sampler aliases the active
        // framebuffer, even when the shader branch guarded by uHasFar is
        // false. The previous frustum leaves a merge texture in these
        // uniforms, so explicitly bind the opposite ping-pong target before
        // starting the next frustum.
        const inactiveMerge = currentMerge === this.mergeTargets[0]
          ? this.mergeTargets[1]
          : this.mergeTargets[0];
        this.mergeMaterial.uniforms.uFarRadiance.value = inactiveMerge.textures[0];
        this.mergeMaterial.uniforms.uFarTransmit.value = inactiveMerge.textures[1];
      }
