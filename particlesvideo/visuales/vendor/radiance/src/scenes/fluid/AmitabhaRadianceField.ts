...
const BALANCED_FIELD_EXTENT = 512;
...
const displayFragmentShader = /* glsl */ `
  ...
  /**
   * Catmull-Rom reconstruction of the field, via nine hardware-filtered taps.
   *
   * The field cannot simply be given more texels: its cascade ray targets grow
   * with the square of the extent, so 2048 would need about 1.2 GB of render
   * targets against 288 MB at 1024. One field texel therefore covers ~2.7
   * output pixels on this show's master, and the field has to be magnified.
   *
   * Plain bilinear magnification is only C0 continuous: its gradient jumps at
   * every texel boundary. The resolve below then applies an unsharp mask, and
   * amplifying a gradient discontinuity is precisely what turned a shallow,
   * high-contrast diagonal into a hard staircase whose steps measured one
   * field texel. Catmull-Rom is C1, so there is no gradient break left at the
   * texel grid for the sharpening to seize on, and unlike a smoothstep-warped
   * bilinear it introduces no plateaus and keeps the edge genuinely sharp.
   */
  vec3 sampleFieldSmooth(vec2 uv) {
    vec2 position = uv / uFieldTexel;
    ...
    // Fold the two inner taps of each axis into one filtered fetch.
    vec2 w12 = w1 + w2;
    vec2 uv0 = (centre - 1.0) * uFieldTexel;
    vec2 uv12 = (centre + w2 / w12) * uFieldTexel;
    vec2 uv3 = (centre + 2.0) * uFieldTexel;

    vec3 result = vec3(0.0);
    result += texture(uIrradiance, vec2(uv0.x, uv0.y)).rgb * (w0.x * w0.y);
    result += texture(uIrradiance, vec2(uv12.x, uv0.y)).rgb * (w12.x * w0.y);
    result += texture(uIrradiance, vec2(uv3.x, uv0.y)).rgb * (w3.x * w0.y);
    result += texture(uIrradiance, vec2(uv0.x, uv12.y)).rgb * (w0.x * w12.y);
    result += texture(uIrradiance, vec2(uv12.x, uv12.y)).rgb * (w12.x * w12.y);
    result += texture(uIrradiance, vec2(uv3.x, uv12.y)).rgb * (w3.x * w12.y);
    result += texture(uIrradiance, vec2(uv0.x, uv3.y)).rgb * (w0.x * w3.y);
    result += texture(uIrradiance, vec2(uv12.x, uv3.y)).rgb * (w12.x * w3.y);
    result += texture(uIrradiance, vec2(uv3.x, uv3.y)).rgb * (w3.x * w3.y);
    // Catmull-Rom's negative lobes can undershoot past black on a hard edge.
  ...
  void main() {
    ...
    // Two texels of margin: the Catmull-Rom kernel reaches that far, and its
    // outer taps must not wrap around the field border.
    vec2 sampleUv = clamp(fieldUv, uFieldTexel * 2.0, vec2(1.0) - uFieldTexel * 2.0);
    vec3 radiance = vec3(0.0);
    if (!outsideField) {
      ...
      // The dominant term of the resolve, and the one the sharpening acts on,
      // so this is the tap that has to be gradient-continuous. The neighbours
      // below stay single bilinear fetches: they only steer the kernel and
      // form wide averages, where a texel-scale kink is already negligible.
      vec3 center = sampleFieldSmooth(sampleUv);
      ...
      // Keep this blend conservative. Weighting the directional resolve by how
      // anisotropic the edge is looks right on a shadow boundary but is wrong
      // here: it cannot tell a step edge from a ridge, so it smooths along thin
      // radial streaks and makes the transport's angular spokes read as a
      // starburst around small bright emitters. Stair-stepping is fixed at its
      // source in the geometry coverage instead, not by leaning on this.
...
export class AmitabhaRadianceField {
  ...
  setQuality(quality: 'high' | 'balanced' | 'safe'): void {
    const resolution = quality === 'high'
      ? HIGH_FIELD_EXTENT
      : quality === 'balanced'
        ? BALANCED_FIELD_EXTENT
        : SAFE_FIELD_EXTENT;
  ...
  private renderFrustum(frustumIndex: number): void {
    ...
    // `uHasFar` is dynamic, so WebGL still treats the dormant far samplers as
    // active on the first merge pass. A previous frustum can leave them bound
    // to the next frustum's first write target, creating a texture/FBO feedback
    // loop. Rebind them to the opposite ping-pong target for every frustum.
    const firstMergeIndex = (this.cascadeCount - 1) % 2;
    const dormantFar = this.mergeTargets[1 - firstMergeIndex];
    this.mergeMaterial.uniforms.uFarRadiance.value = dormantFar.textures[0];
    this.mergeMaterial.uniforms.uFarTransmit.value = dormantFar.textures[1];
