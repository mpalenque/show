...
const publicAssetUrl = (name) => {
  return new URL(radianceAssetPath(`fluid/${name}`), document.baseURI);
};
...
export class KotFluidWorkerClient {
  ...
  /** Complete all previously dispatched commands before changing control owner. */
  drain() {
    this.cancelQueuedStep();
    if (this._disposed) return Promise.reject(new Error('Fluid worker disposed.'));
    if (this.error) return Promise.reject(new Error(this.error));
    const id = ++this._barrierId;
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this._barriers.delete(id);
        reject(new Error('Fluid worker did not acknowledge the scene transition.'));
      }, 10000);
      this._barriers.set(id, { resolve, reject, timer });
      this.worker.postMessage({ type: 'barrier', id });
    });
  }

  get snapshotAgeMs() {
    return this._hasSnapshot ? Math.max(0, performance.now() - this._lastSnapshotAt) : 0;
  }
  ...
  dispose() {
    ...
    this._disposed = true;
    for (const waiter of this._barriers.values()) {
      clearTimeout(waiter.timer);
      waiter.reject(new Error('Fluid worker disposed.'));
    }
  ...
  _handleWorkerMessage(message) {
    if (message.type === 'barrier') {
      const waiter = this._barriers.get(message.id);
      if (waiter) {
        clearTimeout(waiter.timer);
        this._barriers.delete(message.id);
        waiter.resolve();
      }
      return;
    }
